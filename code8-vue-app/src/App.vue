  ...
  <div v-else class="flex h-screen bg-gray-50 font-sans print:block print:h-auto print:bg-white">
    ...
    <aside :class="[
      ...
      <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        <router-link v-if="isStaff()" to="/roster" active-class="bg-gray-800 text-code8-gold border-r-4 border-code8-gold" class="flex items-center gap-3 px-4 py-3 rounded text-base md:text-sm font-medium text-gray-300 hover:bg-gray-800 hover:text-white transition-colors">
          <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
          <span v-if="!isCollapsed || mobileOpen">Athlete Roster</span>
        </router-link>
        <router-link to="/presentation" active-class="bg-gray-800 text-code8-gold border-r-4 border-code8-gold" class="flex items-center gap-3 px-4 py-3 rounded text-base md:text-sm font-medium text-gray-300 hover:bg-gray-800 hover:text-white transition-colors">
        ...
        <router-link v-if="isStaff()" to="/testing" active-class="bg-gray-800 text-code8-gold border-r-4 border-code8-gold" class="flex items-center gap-3 px-4 py-3 rounded text-base md:text-sm font-medium text-gray-300 hover:bg-gray-800 hover:text-white transition-colors">
          ...
          <span v-if="!isCollapsed || mobileOpen">Testing</span>
        </router-link>
        <router-link v-if="isStaff()" to="/evaluation" active-class="bg-gray-800 text-code8-gold border-r-4 border-code8-gold" class="flex items-center gap-3 px-4 py-3 rounded text-base md:text-sm font-medium text-gray-300 hover:bg-gray-800 hover:text-white transition-colors">
        ...
        <div v-if="isStaff()" class="pt-4 mt-4 border-t border-gray-700">
          <p class="px-4 mb-2 text-xs font-semibold uppercase tracking-wider text-gray-500" v-if="!isCollapsed || mobileOpen">Admin</p>
          <router-link to="/manage-athletes" active-class="bg-gray-800 text-code8-gold border-r-4 border-code8-gold" class="flex items-center gap-3 px-4 py-3 rounded text-base md:text-sm font-medium text-gray-300 hover:bg-gray-800 hover:text-white transition-colors">
            ...
            <span v-if="!isCollapsed || mobileOpen">Manage Athletes</span>
          </router-link>
          <router-link v-if="authStore.userRole === 'admin'" to="/admin" active-class="bg-gray-800 text-code8-gold border-r-4 border-code8-gold" class="flex items-center gap-3 px-4 py-3 rounded text-base md:text-sm font-medium text-gray-300 hover:bg-gray-800 hover:text-white transition-colors">
